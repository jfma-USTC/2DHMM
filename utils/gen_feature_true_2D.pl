...
die "perl *.pl <in-file-list> <output-dir> [<shift_right==8> <shift_down==8>]\n".
	"usage: <in-file-list>   --file_list, each line store one data path\n".
	"       <output-dir>     --where all temp-data will be saved\n".
	"       <shift-right>    --optional, step size scan rightward, default 8\n".
	"       <shift-down>     --optional, step size scan downward, default 8\n".
	"e.g. perl ".__FILE__." train_file_list.txt train_feature 8 10\n" if not (@ARGV == 2 or @ARGV == 4);
...
my $pca_MVN_htk_dir = $output_dir.'/'."pca50_MVN";
my $pca_matrix      = $output_dir.'/'."pca.dat";
my $mean_file       = $output_dir.'/'."feature_50.mean";
my $var_file        = $output_dir.'/'."feature_50.var";
my $block_num_info  = $output_dir.'/'."block";
my $shift_right     = 8;
my $shift_down      = 8;
my $raw_feat_dim    = 128;
...
my $BIN_DIR = $ENV{'PWD'}.'/'."bin";
(-e $BIN_DIR) or die "[./bin] dir not exist in current directory! program exit.\n";
...
for(my $i=0;$i<$num_split;$i++)
{
	...
	if($pid==0) {	
		...
		system("$BIN_DIR/feture_extract_2D_log_block_num $scp $raw_htk_dir $block_num_info $shift_right $shift_down 1");
		exit(0);
	}
	push @pids,$pid;
}
...
unlink $block_num_info;
system("$BIN_DIR/log_block_num_true_2d $file_list $block_num_info $shift_right $shift_down");
...
for(my $i=1;$i<=$num_split;$i++)
{
	 ...
	 if($pid==0)
	 {
		 ...
		 system("$BIN_DIR/pca_main_split $scp $part_pca $raw_feat_dim 50 0");
		 exit(0);
	 }
	 push @pids2,$pid;
}
...
system("$BIN_DIR/pca_main_split $part_pca_lst $pca_matrix $raw_feat_dim 50 1");
...
for(my $i=1;$i<=$num_split;$i++)
{
	 ...
	 if($pid==0)
	 {
		 ...
		 system("$BIN_DIR/pca_main_split $scp $pca_matrix $raw_feat_dim 50 2");
		 exit(0);
	 }
	 push @pids2,$pid;

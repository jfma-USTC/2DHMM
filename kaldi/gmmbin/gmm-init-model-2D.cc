...
void InitAmGmm_2D(const BuildTreeStatsType &stats,
...
void InitAmGmmFromOld_2D(const BuildTreeStatsType &stats,
                      const EventMap &to_pdf_map,
  ...
  {  // Read old_gm_gmm
    bool binary_in;
    TransitionModel_2D old_trans_model;
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        ...
        "Usage:  gmm-init-model-2D [options] <tree-in> <tree-stats-in> <topo-file> <model-out> [<old-tree> <old-model>]\n"
        "e.g.: \n"
        "  gmm-init-model-2D tree treeacc topo 1.mdl\n"
        "or (initializing GMMs with old model):\n"
        "  gmm-init-model-2D tree treeacc topo 1.mdl prev/tree prev/30.mdl\n";
    ...
    TransitionModel_2D trans_model(ctx_dep, topo);
    
    // Now, the summed_stats will be used to initialize the GMM.
    AmDiagGmm am_gmm;
    if (old_tree_filename.empty())
      InitAmGmm_2D(stats, to_pdf, &am_gmm, trans_model, var_floor);  // Normal case: initialize 1 Gauss/model from tree stats.
    else {
		//由于绑定中暂时用不上，这部分代码尚未被verified
      InitAmGmmFromOld_2D(stats, to_pdf,

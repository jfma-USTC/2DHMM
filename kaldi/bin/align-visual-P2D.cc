	...
	try {
		...
		while (std::getline(bmp_path_stream, this_bmp_path)) {
			...
			if (alignment_reader.HasKey(file_name)) {
				...
				std::vector<int32> state_list = ali_list; // store trans_state alignment
				std::vector<int32> color_index_list = ali_list; // store color_index alignment
				std::vector<int32> state_appeard; // store trans_state seen in <state_list>
				int shot_times = 0;
				for (size_t i = 0; i < state_list.size(); i++) {
					state_list[i] = trans_model.TransitionIdToTransitionState(ali_list[i]);
					shot_times = 0;
					for (size_t j = 0; j < state_appeard.size(); j++) {
						if (state_list[i] == state_appeard[j]) {
							color_index_list[i] = j;
							shot_times++;
						}
					}
					if (shot_times == 0) {
						state_appeard.push_back(state_list[i]);
						color_index_list[i] = state_appeard.size() - 1;
					}
					else if (shot_times > 1) {
						KALDI_WARN << "state " << state_list[i] << " appear " << shot_times << " times in state_appeard";
						other_error++;
					}
				}
				KALDI_LOG << file_name << "  : " << state_appeard.size() << " states appeared";
				std::string dir;
				std::string imgName;
				std::string imgType;

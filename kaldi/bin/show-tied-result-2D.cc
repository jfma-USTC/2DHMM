...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "\n"
        "Usage:  show-tied-result-2D [options] <phones-symbol-table> <transition/model-file> <tied-result>\n"
        "e.g.: \n"
        " show-tied-result-2D ark:phones.txt final.mdl ark:tied_result.txt\n"
	...
	HmmTopology_2D topo = trans_model.GetTopo();

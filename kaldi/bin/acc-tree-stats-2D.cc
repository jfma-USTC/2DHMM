...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "Usage:  acc-tree-stats-2D [options] <2D-model-in> <features-rspecifier> "
		"<alignments-rspecifier> <tree-accs-out>\n"
        "e.g.: \n"
        " acc-tree-stats-2D 1.mdl scp:train.scp ark:1.ali 1.tacc\n";
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!alignment_reader.HasKey(key)) {
        ...
        AccumulateTreeStats_2D(trans_model,
                            acc_tree_stats_info,
                            alignment,
                            mat,
                            &tree_stats);

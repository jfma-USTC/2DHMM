...
namespace kaldi {
int32 ProcessTopo_2D(const HmmTopology_2D &topo, const std::vector<std::vector<int32> > &questions) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        "Compile questions\n"
        "Usage:  compile-questions-2D [options] <topo> <questions-text-file> <questions-out>\n"
        "e.g.: \n"
        " compile-questions-2D topo questions.txt questions.qst\n";
    bool binary = true;
    int32 P = 1, N = 3;
    ...
    HmmTopology_2D topo;  // just needed for checking, and to get the
    ...
    // 3).
    int32 max_num_pdfclasses = ProcessTopo_2D(topo, questions);
	...
	// 针对不同长度的音素集合，当对音素位置（三音素中为0，1，2）进行提问时，设置的问题为
	// 直接照搬cluster-phone中产生的问题集questions（vector<vector<int32> >）
	...
	// 针对三音素中隐状态进行提问时，设置的问题为
	// 该隐状态是否属于某个隐状态集合，所以当使用三状态HMM描述时，问题集为[[0],[0,1]]
	// 也即当前状态在其HMM中是否属于这一问题集中的某个集合->yes/no

...
int main(int argc, char *argv[]) {
	try {
		...
		const char *usage = "Make alignment of P2D-HMM sample visualable, different color means different states"
			"Usage:  align-visual-P2D [options] <model-in> <bmp-path> <alignments-rspecifier> <output-bmp-path>\n"
			"e.g.: \n"
			" align-visual-P2D --shift-right=5 --shift-down=5 final.mdl all_bmp_path.txt ark:all.ali ali_map_dir\n";
		...
		while (std::getline(bmp_path_stream, this_bmp_path)) {
			...
			if (alignment_reader.HasKey(file_name)) {
				...
				std::vector<int32> pdf_list = ali_list; // store trans_state alignment
				std::vector<int32> color_index_list = ali_list; // store color_index alignment
				std::vector<int32> pdf_appeard; // store trans_state seen in <pdf_list>
				int shot_times = 0;
				std::string pdf_list_str = "";
				for (size_t i = 0; i < pdf_list.size(); i++) {
					pdf_list[i] = trans_model.TransitionIdToPdf(ali_list[i]);
					KALDI_LOG << "ali : " << ali_list[i] << " < --- > pdf : " << pdf_list[i];
					shot_times = 0;
					for (size_t j = 0; j < pdf_appeard.size(); j++) {
						if (pdf_list[i] == pdf_appeard[j]) {
							color_index_list[i] = j;
							shot_times++;
						}
					}
					if (shot_times == 0) {
						pdf_appeard.push_back(pdf_list[i]);
						color_index_list[i] = pdf_appeard.size() - 1;
					}
					else if (shot_times > 1) {
						KALDI_WARN << "state " << pdf_list[i] << " appear " << shot_times << " times in pdf_appeard";
						other_error++;
					}
					pdf_list_str += (std::to_string(pdf_list[i]) + " ");
				}
				KALDI_LOG << file_name << " pdf list : " << pdf_list_str;
				std::string dir;
				std::string imgName;
				std::string imgType;
				...
				if (bmp_save_path.find_last_of("/") == bmp_save_path.size() - 1) {
					ali_bmp_path = bmp_save_path + imgName + "_ali_pdf." + imgType;
				}
				else {
					ali_bmp_path = bmp_save_path + "/" + imgName + "_ali_pdf." + imgType;
				}
				...
				cv::Mat img_3ch;
				if (LineNormedImg.channels() == 1) {
					img_3ch = cv::Mat::zeros(LineNormedImg.rows, LineNormedImg.cols, CV_8UC3);
					std::vector<cv::Mat> channels;
					for (size_t i = 0; i < 3; i++)
						channels.push_back(LineNormedImg);
					cv::merge(channels, img_3ch);
				}
				else if (LineNormedImg.channels() == 3) {
					img_3ch = LineNormedImg;
				}
				else {
					std::cout << "img " << imgName << " has channels: " << LineNormedImg.channels() << " , unable to handle.\n";
					return 0;
				}
				
				std::vector<int> this_rgb;
				// -------- 对二值化图像进行尺度归一（保持长宽不变高度变为‘Reszie_Rows’pixels）-------- //
				int frameWidth = 40,  // 水平切帧的帧宽
				...
				//cv::copyMakeBorder(img_3ch, img_3ch, 0, 0, leftPad, rightPad, cv::BORDER_CONSTANT, cv::Scalar(255, 255, 255));

				int Sample_Num_Rows = (frameHeight - blockHeight) / shift_down + 1; //竖直方向上应该被切出来的帧数（有几行帧）
				int Sample_Num_Cols = (img_3ch.cols - frameWidth) / shift_right + 1; //水平方向上应该被切出来的帧数（有几列帧）
				int Sample_Num_Tol = Sample_Num_Rows * Sample_Num_Cols; //水平方向上应该被切出来的帧数（有几列帧）
				if (color_index_list.size() != Sample_Num_Tol) {
				...
				cv::Size resize_size_to_color(color_map.cols, color_map.rows);
				cv::resize(img_3ch, img_3ch, resize_size_to_color);
				// 原图首先高度resize到60之后再左右各加20，然后再resize成color_map一样大小

				for (size_t j = 0; j < Sample_Num_Cols; j++) {
					for (size_t i = 0; i < Sample_Num_Rows; i++) {
						int32 ali_index = i + j * Sample_Num_Rows;
						int32 ali = color_index_list[ali_index];
				...
				KALDI_WARN << "AlignVisualP2D: no alignment info for utterance " << file_name;
				no_ali++;
			}
		}
